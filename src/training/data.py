...
# FIXME 在封装前就将图片划分成子图
# FIXME 裁剪函数
def crop_and_merge(images, subfig_size):
...
class AutoCrop(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        self.size, h_crop_num, w_crop_num, resize_flag = _setup_size_all_resize(img=img, max_subfigs=self.max_subfigs,
                                                                     subfig_size=self.subfig_size)
        if resize_flag == 1:
            return ttf.resize(img, self.size)
...
class CsvDataset(Dataset):
    def __init__(self, input_filename, transforms, img_key, caption_key, sep="\t", tokenizer=None):
        ...
        self.transforms_global = transforms
    ...
    def __getitem__(self, idx):
        ...
        ## token mask, one image equal to 48 tokens
        attention_mask = torch.zeros(24*24 + 1, dtype=torch.long)
        # 裁剪成子图
        ...
        attention_mask[:(figs_num + 1) * 48 + 1] = 1
